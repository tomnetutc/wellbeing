...
export default function ZTHeatMap(): JSX.Element | null {
  ...
  useEffect(() => {
    ...
    let cache_key = "ZTHMAP-" + cacheMaxYear;
    let old_cache = "ZTHMAP-" + (intOfYear - 1).toString();
    if (localStorage.getItem(cache_key) == null) {
      ...
      csv(
        ...
        for (const property in stateValue) {
          let calcValue = Number(
            ((stateNumerator[property] * 100) / stateValue[property]).toFixed(1)
          );
          if (calcValue) {
            evaluatedScore.push([property, calcValue]);
          }
        }
        ...
        localStorage.setItem("ztFinalValues", JSON.stringify(stateOrder));
        localStorage.setItem(
          "ztQuintileVals",
          JSON.stringify(colorQuintileValues)
        );
        localStorage.setItem("ztmYear", JSON.stringify(mYear));
        localStorage.setItem("zthmdata", JSON.stringify(data));
        localStorage.setItem("ztlayout", JSON.stringify(layout));
        localStorage.setItem("ztconfig", JSON.stringify(config));
        localStorage.setItem("ztTopFive", JSON.stringify(topFiveStates));
        localStorage.setItem("ztBotFive", JSON.stringify(bottomFiveStates));
      });
      ...
      let fvals = JSON.parse(localStorage.getItem("ztFinalValues") || "");
      let qvals = JSON.parse(localStorage.getItem("ztQuintileVals") || "");
      let hmdata = JSON.parse(localStorage.getItem("zthmdata") || "");
      let hmxyear = JSON.parse(localStorage.getItem("ztmYear") || "");
      let hmlayout = JSON.parse(localStorage.getItem("ztlayout") || "");
      let hmconfig = JSON.parse(localStorage.getItem("ztconfig") || "");
      let tfive = JSON.parse(localStorage.getItem("ztTopFive") || "");
      let bfive = JSON.parse(localStorage.getItem("ztBotFive") || "");
  ...
  return (
    <Container>
      ...
      <Row>
        <Col lg={8}>
          <div>
            <h4 className="fw-bold">Zero trip-making by states</h4>
            <p>
              The map below shows the percentage of zero trip-makers (people who
              make no trips during a day) by state for a selected year. Use the
              slider to move between years.
            </p>
          </div>
          <RangeSlider
            variant="danger"
          ...
          <div style={{ transform: "translate(0px,50px)" }}>
            <Row className="text-center">
              <Col lg={3}></Col>
              <Col className="border">Percent of zero trip-makers (%)</Col>
              <Col lg={3}></Col>
            </Row>
            <Row className="text-center">
              <Col lg={3}></Col>
        ...
        <Col lg={4}>
          ...
          <p>
            See below the states with highest and lowest zero trip-making
            percentages for the year selected.
            <i className="ms-1">
            ...
            return (
              <Row key={idx} style={{ height: "30px", paddingLeft: "12px" }}>
                <Col className="border" style={{ backgroundColor: "#FBFCF2" }}>
                  <span>{finalValues[finalValues.length - 1 - idx][0]}</span>
                </Col>
                <Col
                  lg={3}
                ...
                >
                  <span>
                    {finalValues[finalValues.length - 1 - idx][1] + "%"}
                  </span>
                </Col>
                <Col lg={3}></Col>
              </Row>
